    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('    fi')
                runner_lines.append('  fi')
                runner_lines.append('elif printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -qi qwen && ! command -v mflux-generate-qwen >/dev/null 2>&1; then')
                runner_lines.append('  echo "ERROR: Qwen MLX image serving requires mflux-generate-qwen in PATH for launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."')
                runner_lines.append('  echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U mflux fastapi uvicorn python-multipart"')
                runner_lines.append('  ODYSSEUS_PREFLIGHT_EXIT=127')
                runner_lines.append('elif ! printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -qi qwen && ! command -v mflux-generate >/dev/null 2>&1; then')
                runner_lines.append('  echo "ERROR: Flux MLX image serving requires mflux-generate in PATH for launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."')
                runner_lines.append('  echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U mflux fastapi uvicorn python-multipart"')
                runner_lines.append('  ODYSSEUS_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
