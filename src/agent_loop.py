...
- "Cookbook" = the LLM-serving subsystem (NOT chat sessions, NOT a recipe app). Routing:
  ...
  • Launching/debugging a difficult model where the user wants you to handle the engine/driver/command work → prefer `launch_model_agent`. It resolves the official HuggingFace page/metadata, picks an engine/command for the selected host, launches through Cookbook, polls readiness, tails only the new session on failure, and applies bounded structured retries.
...
_DOMAIN_TOOL_MAP = {
    ...
    "cookbook": {"download_model", "serve_model", "launch_model_agent", "serve_preset", "list_serve_presets", "list_served_models", "stop_served_model", "tail_serve_output", "list_downloads", "cancel_download", "search_hf_models", "list_cached_models", "list_cookbook_servers", "adopt_served_model"},
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "launch_model_agent": "- ```launch_model_agent``` — Autonomous Cookbook launch/debug loop. Args (JSON): {\"repo_id\":\"...\", \"host\":\"ajax\"?, \"engine\":\"vllm|sglang|llama.cpp|mlx\"?, \"dry_run\":false?, \"max_attempts\":2?}. It looks up the official HuggingFace page/metadata, infers engine/command for the host, launches through Cookbook tracking, polls readiness, tails only the new session on failure, applies structured retry suggestions, and stops after a bounded retry limit. Use this for difficult model launches instead of manually guessing commands/drivers.",
